...
class JELoader {
    ...
    fun init(path: String) {
        ...
        val serverPropertiesPath = Paths.get(path, "server.properties")
    ...
    fun getGenerator(type: String, seed: Long, biome: String = ""): JEGenerator {
        ...
        synchronized(storageAccess) {
            ...
            val jeGenerator = generators[name]
            if (jeGenerator != null) return jeGenerator

            val completableFuture = CompletableFuture<JEGenerator>()
            val task = Runnable {
                completableFuture.complete(getGeneratorSync(type, seed, biome))
            }
            thread.addTask(task)
